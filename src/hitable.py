...
class rotate_z(hitable):
	def __init__(self,p: hitable, angle: float):  
		...
		for i in range(2):
			for j in range(2):
				for k in range(2):
					...
					y = j*self.bbox._max[1] + (1-j)*self.bbox._min[1]
					z = k*self.bbox._max[2] + (1-k)*self.bbox._min[2]
					newx = self.cos_theta*x - self.sin_theta*y;
					newy = self.sin_theta*x + self.cos_theta*y;
		...
		self.bbox = aabb(self._min_, self._max_)
	def hit(self,r:ray, t_min:float, t_max: float):
		new_origin_x = self.cos_theta*r.origin[0] + self.sin_theta*r.origin[1]
		# this ensures that new_origin_y populates a new value with no reference to origin r.origin[1]
		# this ensures that original ray data is not polluted when we stick it into new ray
		# we do this instead copy or deepcopy because both of those commands are incredibly expensive
		# and we only care about affecting numerical primitives here, honestly python needs a better way to do this...
		new_origin_y = -self.sin_theta*r.origin[0] + self.cos_theta*r.origin[1]
		new_origin_z = 1.0 * r.origin[2]
		new_direction_x  = self.cos_theta*r.direction[0] + self.sin_theta*r.direction[1]
		new_direction_y = -self.sin_theta*r.direction[0] + self.cos_theta*r.direction[1]
		new_direction_z = 1.0 * r.direction[2]
		new_time = r.time * 1.0
		...
		is_hit,rec = self.p.hit(rotated_r, t_min, t_max)
		if (is_hit):
			p_x = self.cos_theta*rec.p[0] - self.sin_theta*rec.p[1];
			p_y = self.sin_theta*rec.p[0] + self.cos_theta*rec.p[1];
			normal_x = self.cos_theta*rec.normal[0] - self.sin_theta*rec.normal[1];
			normal_y = self.sin_theta*rec.normal[0] + self.cos_theta*rec.normal[1];
			rec.p[0], rec.p[1] =p_x, p_y;
			rec.normal[0],rec.normal[1] = normal_x, normal_y;
			return (True,rec);
		else: 
			return (False,rec);
		def bounding_box(self,t0: float, t1: float):
